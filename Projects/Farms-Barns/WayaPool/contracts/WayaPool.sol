...
contract WayaPool is Ownable, Pausable {
    ...
    address public boostContract; // boost contract used in TaskMaster.
     ...
     * @param _pid: waya pool ID in ChiefFarmer
     */
    constructor(
        IERC20 _wayaToken,
    ...
    /**
     * @notice Deposits a dummy token to `CHIEF_FARMER` CF.
    ...
    /**
     * @notice Checks if the msg.sender is either the waya owner address or the operator address.
     */
    modifier onlyOperatorOrWayaOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or waya owner");
        _;
    }
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                if (!freeOverdueFeeUsers[_user] && ((user.lockEndTime + UNLOCK_FREE_DURATION) < block.timestamp)) {
                    ...
                    wayaToken.safeTransfer(treasury, currentOverdueFee);
                ...
                if (currentPerformanceFee > 0) {
                    wayaToken.safeTransfer(treasury, currentPerformanceFee);
     ...
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrWayaOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    function depositOperation(
        ...
        // Harvest tokens from TaskMaster.
        harvest();

        // Handle stock funds.
        if (totalShares == 0) {
            uint256 stockAmount = available();
            wayaToken.safeTransfer(treasury, stockAmount);
        }
        // Update user share.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (!freeWithdrawFeeUsers[msg.sender] && (block.timestamp < user.lastDepositedTime + withdrawFeePeriod)) {
            ...
            wayaToken.safeTransfer(treasury, currentWithdrawFee);
    ...
    /**
     * @notice Harvest pending WAYA tokens from TaskMaster
     */
     ...
     * @dev Callable by the contract admin.
     */
    function setVWayaContract(address _VWaya) external onlyAdmin {
