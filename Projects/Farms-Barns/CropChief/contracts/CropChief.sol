...
contract CropChief is Ownable, ReentrancyGuard {
    ...
    // The block number when WAYA mining ends.
    ...
    function initialize(
        ...
        require(msg.sender ==  CROPCHIEF_FACTORY, "Not factory");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 WAYA_Reward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare + (WAYA_Reward * PRECISION_FACTOR) / stakedTokenSupply;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 WAYA_Reward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (WAYA_Reward * PRECISION_FACTOR) / stakedTokenSupply;
        lastRewardBlock = block.number;
    }
